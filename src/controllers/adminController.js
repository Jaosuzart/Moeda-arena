...
const adicionarTokensManualmente = async (req, res, next) => {
  try {
    ...
    const sucessoDb = await usuarioModel.adicionarTokens(usuarioId, Number(quantidade));
    if (!sucessoDb) {
      return erro(res, 'Usuário não encontrado.', 404);
    }

    return sucesso(res, { mensagem: `${quantidade} tokens adicionados com sucesso ao usuário ID ${usuarioId}.` });
  } catch (err) {
    next(err);
  }
};
...
module.exports = { isAdmin, listarUsuarios, adicionarTokensManualmente, alterarStatusUsuario };
