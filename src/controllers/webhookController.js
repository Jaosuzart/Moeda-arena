...
const processarNotificacao = async (req, res) => {
  ...
  try {
    ...
    if (tipoEvento !== "payment" || !idPagamento) {
      logger.debug("Webhook ignorado: tipo não é payment.", {
        tipo: tipoEvento,
      });
      return;
    }
    ...
    if (processado) {
      logger.info(
        "Pagamento já processado anteriormente. Ignorando duplicata.",
        {
    ...
    if (creditado) {
      ...
      logger.error(
        "Falha ao creditar tokens: usuário não encontrado no banco.",
        {
          paymentId: idPagamento,
          usuarioId,
        },
