...
const loginGoogle = async (req, res, next) => {
  try {
    ...
    if (!usuario) {
      ...
      logger.info("Novo registro via Google realizado.", {
...
const atualizarPerfil = async (req, res, next) => {
  try {
    const { nome, cpf, localidade, chave_pix, cartao_final, senhaConfirmacao } =
      req.body;
    ...
    if (!senhaConfirmacao) {
      return erro(
        res,
        "A senha de confirmação é obrigatória para salvar as alterações.",
        403,
        "SENHA_OBRIGATORIA",
      );
    }
    ...
    const sucessoAtualizacao = await usuarioModel.atualizarPerfil(
      req.usuario.id,
      {
        nome: nome.trim(),
...
const definirSenha = async (req, res, next) => {
  try {
    ...
    return sucesso(res, {
      mensagem:
        "Senha de segurança definida com sucesso! Agora você pode atualizar seu perfil.",
    });
  } catch (err) {
    next(err);
  }
...
const solicitarRecuperarSenha = async (req, res, next) => {
  try {
    ...
    if (!usuario) {
      return sucesso(res, {
        mensagem:
          "Se o e-mail estiver cadastrado, um link de redefinição de senha será enviado em instantes.",
      });
    }
    ...
    logger.info("Solicitação de recuperação de senha gerada.", {
      usuarioId: usuario.id,
    });
    return sucesso(res, {
      mensagem:
