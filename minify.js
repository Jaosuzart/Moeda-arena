...
function runMinify() {
  try {
    ...
    fs.writeFileSync(
      path.join(publicDir, "style.min.css"),
      minifiedCSS,
      "utf8",
    );
...
if (require.main === module) {
  ...
  try {
    const res = runMinify();
    console.log(
      `style.css minified. Size reduced from ${res.cssOriginal} to ${res.cssMinified} bytes.`,
    );
    console.log(
      `main.js minified. Size reduced from ${res.jsOriginal} to ${res.jsMinified} bytes.`,
    );
