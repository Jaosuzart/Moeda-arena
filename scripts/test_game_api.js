...
const reqGet = http.request(
  `http://localhost:${config.port}/api/games/saldo/${EMAIL}?api_key=${SECRET}`,
  { method: "GET" },
  (res) => {
    let data = "";
    res.on("data", (chunk) => (data += chunk));
    res.on("end", () => {
      logger.info("--- GET SALDO ---", {
        status: res.statusCode,
        resposta: data,
      });
      const postData = JSON.stringify({ email: EMAIL, quantidade: 1 });
      const reqPost = http.request(
        `http://localhost:${config.port}/api/games/consumir`,
        {
          method: "POST",
          headers: {
        ...
        (resPost) => {
          ...
          resPost.on("end", () => {
            logger.info("--- POST CONSUMIR ---", {
              status: resPost.statusCode,
              resposta: dataPost,
            });
          });
        },
